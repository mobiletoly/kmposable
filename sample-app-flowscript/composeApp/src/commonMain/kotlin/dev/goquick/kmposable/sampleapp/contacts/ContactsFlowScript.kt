...
): Job = runScript(scriptScope, onTrace) {
    while (true) {
...
private suspend fun NavFlowScriptScope<ContactsFlowEvent, DefaultStackEntry<ContactsFlowEvent>>.loadContacts(
    repository: ContactsRepository
) {
    val listNode = navState.value.root as ContactsListNode
    runCatchingNodeCall(
